/*
 ...
 Purpose: MonoBehaviour that handles the dynamic difficulty adjustment system.
...
namespace ProjectAssets.Scripts.Gameplay.Difficulty_Adjustment
{
    ...
    {
        [Range(4, 15)] public int boardSize = 4;
        [Range(0, 100)] public int moves = 4;
        ...
        [Header(" ")] public double levelRatingDebug = 0;
            ...
            else
            {
                if (GameManager.Instance.hasDDA)
                {
                    randomBoardSize =
                        rating < 11
                            ? 4
                            : BoardSizeRatingRange(
                                rating); // Disable this if you want a full random board everytime (NO DDA)
                    randomBoardSize = FitMovesOnBoard(randomBoardSize, moveNumber);
                    parameters.SetBoardSize(
                        randomBoardSize); // Always the initial BoardSize // Use data from previous levels
                }
                else
                {
        ...
        int SetupMoves(int moveOut)
        {
            ...
            {
                var average = (double)((SaveManager.Instance.playerProfile.levelsPlayed[0].expectedMoves +
                                        SaveManager.Instance.playerProfile.levelsPlayed[1].expectedMoves +
                                        SaveManager.Instance.playerProfile.levelsPlayed[2].expectedMoves) / 3.0) * .7;
                return (int)(average + moveOutput);
            }

            if (previousLevel.expectedMoves + moveOutput <= 0)
            {
                return 7;
            }
        ...
        public double SetLevelRating(double rating, double moveIncrement, bool previousLevelWon)
        {
            ...
            newRating = newRating <= 0 ? 5 : newRating;
            return newRating;
        }
        ...
        public void SetupDifficultyParameters() // we can edit this to for two builds With and wuthout DDA
        {
            ...
            if (SaveManager.Instance.playerProfile.levelsPlayed.Count > 2 && isDDAActive)
            {
                ...
                Debug.Log($"{previousLevel.expectedMoves} {previousLevel.allottedTime}");
            ...
            if (SaveManager.Instance.playerProfile.levelsPlayed.Count > 2 && isDDAActive)
            {
                puzzleRating = (int)SetLevelRating(SaveManager.Instance.playerProfile.currentRating, moveOutput,
                    SaveManager.Instance.playerProfile
                        .levelsPlayed[SaveManager.Instance.playerProfile.levelsPlayed.Count - 1].won);
            }
            else if (!isDDAActive)
            {
        ...
        {
            var playerScore =
                parameters.SetScoreOfPlayer(pMoves, eMoves, pTime,
                    pMoveOnSuggestedPath); // ref is a pointer reference to the variables
            playerScore = won ? playerScore : -(playerScore);
            return playerScore;
        }

        public void
            ComputeLevelScore() // invoke after finishing the level this will compute the score and the rating for the next level
        {
            ...
            if (levelGenerated.playerMove == 0)
            {
                levelGenerated.playerMove = 1;
            }

            var playerScore = parameters.SetPlayerScore(ref levelGenerated.playerMove,
                ref levelGenerated.playerRemainingTime,
                levelGenerated.playerMoveOnSuggestedPath); // ref is a pointer reference to the variables
            ...
            {
                newPlayerRating =
                    (levelGenerated.playerRating + (nextLevelRating * .01f) / 1) +
                    (playerScore / 2.5); // This is based from elo rating of chess
            }
            ...
            else
            {
                if (playerWon)
                    newPlayerRating =
                        (levelGenerated.playerRating + (nextLevelRating * .01f) / currentPlayer.gamesPlayed) +
                        (playerScore / 6); // divided by 6 so player Rating will not go up to much
                else
                    newPlayerRating =
                        (levelGenerated.playerRating + (nextLevelRating * .01f) / currentPlayer.gamesPlayed) +
                        (playerScore);
            ...
            newPlayerRating = newPlayerRating <= 5 ? newPlayerRating = 5 : newPlayerRating;
            ...
            // Have a seed the saves the info
            var details = new LevelDetails(playerWon, levelGenerated.boardSize, levelGenerated.allottedTime,
                levelGenerated.expectedMoves, levelGenerated.suggestedPath, levelGenerated.levelRating,
                levelGenerated.playerRating, levelGenerated.playerMove, levelGenerated.playerMoveOnSuggestedPath,
                levelGenerated.playerRemainingTime, levelGenerated.playerScore, levelGenerated.levelScore);
        ...
        public double NextLevelRating()
        {
            ...
            Debug.Log(
                $"Next Level Rating/Player Rating Computation: CurrentPR: {currentPlayerRating} levelRating: {levelRating}  multiplier: {multiplier} ratingResult: {ratingResult} playerScore: {playerScore}  result: {result} ");
            return result;
        }


        // Computation for the Player Rating based on Elo Rating System
        public double AddPlayerRating(int gamesPlayed, int playerRating, double levelRating, int playerScore,
            bool playerWon)
        {
            var pRating = playerRating;
            if (gamesPlayed < 1)
            {
                pRating = (int)((playerRating + (levelRating * .01f) / 1) +
                                (playerScore / 2.5)); // This is based from elo rating of chess
            }
            // This is based from elo rating of chess
            else
            {
                if (playerWon)
                    pRating = (int)((playerRating + (levelRating * .01f) / gamesPlayed) +
                                    (playerScore / 2.5f)); // divided by 6 so player Rating will not go up to much
                else
                    pRating = (int)((playerRating + (levelRating * .01f) / gamesPlayed) + (playerScore));
            }

            Debug.Log(
                $"Player Rating Computation: PlayerRating: {playerRating} levelRating: {levelRating}  playerScore: {playerScore} games played: {gamesPlayed}  pRating: {pRating} ");
            return pRating;
        }
